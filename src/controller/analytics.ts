...
const abandonedCheckouts = await fetch(
  `https://asmi-9843.myshopify.com/admin/api/2024-10/checkouts.json?status=open`,
  {
    method: 'GET',
    headers: {
...
const completedOrders = await fetch(
  `https://asmi-9843.myshopify.com/admin/api/2024-10/orders.json?status=any&financial_status=paid`,
  {
    method: 'GET',
    headers: {
      'X-Shopify-Access-Token': accessToken,
